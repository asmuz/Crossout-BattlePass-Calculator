...
function onchangeDay() {
  paid = paidLevel();
  if (paid == 0) document.getElementById("range-inputResources").max = 150;
  else document.getElementById("range-inputResources").max = 1960;
...
function onchangeResources() {
  ...
  paid = paidLevel();
  if (paid == 0) level = levelForFreeResources(inputResources);
  else level = levelForResources(inputResources);
